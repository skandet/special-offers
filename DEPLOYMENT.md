...
1. **Vercel 特殊说明**
   - Vercel 是无状态的，每次请求可能会分配到不同的实例
   - SQLite 数据库和上传的图片存储在 `/tmp` 目录，实例销毁后数据会丢失
   - 建议在生产环境中使用以下方案：
     - **云数据库**：使用 Supabase、PlanetScale 或 AWS RDS 等
     - **云存储**：使用 Cloudinary、AWS S3 或 Vercel Blob 存储图片

2. **数据存储**
   - 开发环境：使用 SQLite 数据库，存储在 `/tmp/app.db`
   - 生产环境：建议迁移到云数据库服务
   - 所有用户共享同一套数据
   - 不同浏览器访问的数据是一致的
...
6. **替代部署方案**
   - 如果需要持久化存储，建议使用：
     ...
     - **PythonAnywhere**：https://www.pythonanywhere.com/wiki/Flask
